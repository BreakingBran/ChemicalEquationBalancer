...
equation = Equation(raw_equation, doSolve = False)
...
class EquationBalencerTest(unittest.TestCase):
    ...
    def test_makeWhole(self):
        roundedDenominatorList = [round(x) for x in equation.denominatorList]
        self.assertListEqual(roundedDenominatorList, [3,2,6,1], msg = "The denominator list does not equal")
        roundedmultipliedDenominatorList = [round(x) for x in equation.multipliedDenominatorList]
        self.assertListEqual(roundedmultipliedDenominatorList, [12, 18, 6, 36], msg = "The multipliedDenominatorList list does not equal")
        self.assertListEqual(equation.answerList, [2, 3, 1, 6], msg = "The answerList list does not equal, most likley due to rounding")
        
    def test_outputAnswer(self):
        self.assertEqual(equation.balencedEquation, "2 Na3PO4 + 3 CaCl2 = 1 Ca3(PO4)2 + 6 NaCl", "The balenced string does not print out properly")
